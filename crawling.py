...
def crawling_favorite_blogs(
    blog_info_lst, today=datetime.now().date() - timedelta(days=1)
):
    """
    ...
    for blog_name, blog_dict in blog_info_lst:
        soup = bs(
            requests.get(blog_dict["base_url"] + blog_dict["post_path"]).text,
            "html.parser",
        )
        ...
        for post in posts:
            try:
                if link_info[0] == None:
                    ...
                    link = post.find(
                        link_info[0], attrs={link_info[1]: link_info[2]}
                    ).get("href")

                if (
                    blog_dict["detail_page_is_absolute"] is not None
                    and not blog_dict["detail_page_is_absolute"]
                ):
                ...
                if title_info[1] == None:
                    ...
                    title = post.find(
                        title_info[0], attrs={title_info[1]: title_info[2]}
                    ).get_text()

                if blog_dict["need_enter_detail_page_for_publish_date"]:
                    detail_soup = bs(requests.get(link).text, "html.parser")
                    publish_date_str = detail_soup.find(
                        publish_info[0], attrs={publish_info[1]: publish_info[2]}
                    ).get_text()
                else:
                    if publish_info[1] == None:
                        publish_date_str = post.find(publish_info[0]).get_text()
                    else:
                        publish_date_str = post.find(
                            publish_info[0], attrs={publish_info[1]: publish_info[2]}
                        ).get_text()

                publish_date = datetime.strptime(
                    publish_date_str, blog_dict["datetime_format"]
                )
